...
public class EmpDaoImpl implements EmpDao {
	// 디비와 대화하는 객체를 구한다.
	// Template : 일부분의 처리코드가 완성되어 있어서 빠져있는 부분만 추가하면 사용할수 있는 객체
	// 멀티스레드 환경에서 안전하다. ==> 빈 컨테이너 등록해서 싱글톤 방식으로 써도 된다.
	@Autowired
	private JdbcTemplate jdbcTemplate;
	
	// 테이블의 로우의 컬럼들을 도메인 클래스 멤버변수에 어떻게 옮기는가를 정의한 로직
	...
	public int count() {
		...
		// 결과값이 1개인경우 queryForObject() 메소드를 사용한다.
