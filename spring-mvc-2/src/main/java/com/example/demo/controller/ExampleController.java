...
//			produces = {	"text/plain" }, 			// 결과의 MIME 타입(평범한 문자열을 리턴하겠다)
	...
	// 서블릿처럼 HttpServletRequest, HttpServletResponse 객체를 파라미터로 받을 수 있다.
	// Locale 객체는 브라우저가 신고한 HTTP 헤더정보에서 구한 클라이언트의 지역정보가 담겨 있다.
	@GetMapping("/example2")
	public String example2(HttpServletRequest req, Model model, HttpSession session, Locale locale) {
		...
		//호출될때 세션 객체가 이미 있으면 그대로 사용하고 없으면 새로 만들어서 리턴한다.
		System.out.println(session == req.getSession());
		System.out.println(locale.getCountry() + "," + locale.getLanguage());
		return "example2";
	}

	// 브라우저가 보내는 데이터를 받고자 할 때 VO를 만들어서 이용하거나 Map 객체를 주로 사용한다.
	...
	public ModelAndView example3(
		...
		// 멤버변수가 User 클래스안에 존재하는 경우 자동으로 값이 User 객체의 멤버변수에 주입된다.
		...
		// ModelAndView는 연동할 뷰와 전달할 데이터를 한 번에 취급하는 객체다.
	...
	// @RequestParam 으로 설정했는데 전달되는 파라미터가 없다면 에러가 발생한다. required=false 설정으로 막을 수 있다.
	// http://localhost:8080/example4?id=xx URL과 http://localhost:8080/example4?id=xx&a=21 URL 테스트가능
	@GetMapping("/example4")
	@ResponseBody
...
//		"/" 문자열을 잘라서 배열로 전환
	...
	// 위치보유자 문법
	// URI 부분에 {패스홀더} 설정을 하면 @PathVariable("패스홀더") 애노테이션을 사용하여
	// URI 패스홀더 자리에 문자열을 메소드에 파라미터로 받을 수 있다.
