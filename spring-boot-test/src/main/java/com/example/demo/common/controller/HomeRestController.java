...
// 1. String을 리턴하면 그대로 클라이언트에게 전달됨.
// 2. 객체를 리턴하면 Jackson 라이브러리를 사용해서 JSON 포맷의 문자열로 변경한 후, 클라이언트에게 전달됨.
@RestController
public class HomeRestController {

	// http://localhost:9090/example/?plain
	// @RequestMapping("/") 에노테이션에게 속성값을 전달할 때, 속성키를 명시하지 않으면 자동으로 value 속성키가 사용됨.
	// 연동기술로 URL 문자열을 사용함.
	// 연동은 설정된 모든 속성이 만족해야만 가능함. (And연산)
	...
	public Object exampleObjectToJson() {
		// 리턴값이 객체인 경우, 스프링이 JSON 포맷의 문자열로 변경하여 클라이언트에게 전달함
		return map;
	}
